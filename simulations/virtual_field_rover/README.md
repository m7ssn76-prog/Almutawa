...
## Governed ASA/AOIP bridge

The existing local bridge can ingest the synthetic run summary into the authenticated ASA/AOIP knowledge API as a draft synthetic record. The bridge remains fail-closed for real-farm states and does not change the simulation-only boundary.
