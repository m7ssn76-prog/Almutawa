...
To detect tail truncation as well as record modification, the database also maintains a single signed `hmac-sha256-state-v1` chain checkpoint containing the chained-event count, last chained event ID, and last event hash. Its key is derived through a third domain separated from both the question fingerprint and event-chain constructions. The verifier checks the checkpoint HMAC, event count, chain head, last ID, and SQLite AUTOINCREMENT sequence so deletion of the latest event or deletion/tampering of the checkpoint is detectable in controlled verification. When software is upgraded from the pre-checkpoint chain format, an existing internally consistent chain may receive a checkpoint for its **currently observed state** when the audit key is available; that migration checkpoint does not retroactively prove that no earlier truncation occurred before the checkpoint was established.

These controls are **tamper-evident, not immutable**. A party with sufficient database access and the audit secret could potentially rebuild both the chain and checkpoint, and wholesale replacement of the database together with its metadata is outside the guarantee of this local control. The implementation is not an external timestamp, append-only institutional log, HSM-backed signature, SIEM/WORM store, remote transparency service, or independent notarization. Those stronger guarantees remain external or future controls.
